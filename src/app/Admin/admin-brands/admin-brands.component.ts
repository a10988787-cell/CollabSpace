// src/app/Admin/admin-brands/admin-brands.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-brands.component.css', '../admin-shared.css'],
...
export class AdminBrandsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('brands').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('brands', this.form._id, body)
      : this.svc.create('brands', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('brands', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('brands_chart') as HTMLCanvasElement;
    if (!el) return;
