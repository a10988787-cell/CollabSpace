// src/app/Admin/admin-notifications/admin-notifications.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-notifications.component.css', '../admin-shared.css'],
...
export class AdminNotificationsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('notifications').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('notifications', this.form._id, body)
      : this.svc.create('notifications', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('notifications', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('notifications_chart') as HTMLCanvasElement;
    if (!el) return;
