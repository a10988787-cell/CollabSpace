// src/app/Admin/admin-campaigns/admin-campaigns.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-campaigns.component.css', '../admin-shared.css'],
...
export class AdminCampaignsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('campaigns').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('campaigns', this.form._id, body)
      : this.svc.create('campaigns', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('campaigns', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('campaigns_chart') as HTMLCanvasElement;
    if (!el) return;
