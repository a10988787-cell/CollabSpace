...
@Component({
  selector: 'app-admin-collab-posts',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-collab-posts.component.html',
  styleUrls: ['../admin-shared.css'],
  encapsulation: ViewEncapsulation.None,
})
export class AdminCollabPostsComponent implements OnInit {
  items: any[] = [];
  loading  = true;
  saving   = false;
  showModal = false;
  ...
  load(): void {
    this.loading = true;
    this.svc.list('collab-posts', { search: this.search||undefined, page: this.page }).subscribe({
      next: (r:any) => {
        this.items   = r.data || [];
  ...
  save(): void {
    ...
    const ob$ = this.isEdit
      ? this.svc.update('collab-posts', this.form._id, this.form)
      : this.svc.create('collab-posts', this.form);
    ob$.subscribe({
  ...
  doDelete(): void {
    this.svc.remove('collab-posts', this.delId).subscribe({
