...
@Component({
  ...
  styleUrls: ['./admin-settings.component.css', '../admin-shared.css'],
...
export class AdminSettingsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('settings').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('settings', this.form._id, body)
      : this.svc.create('settings', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('settings', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('settings_chart') as HTMLCanvasElement;
    if (!el) return;
