// src/app/Admin/admin-creators/admin-creators.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-creators.component.css', '../admin-shared.css'],
...
export class AdminCreatorsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('creators').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('creators', this.form._id, body)
      : this.svc.create('creators', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('creators', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('creators_chart') as HTMLCanvasElement;
    if (!el) return;
