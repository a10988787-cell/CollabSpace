...
@Component({
  selector: 'app-admin-brand-profiles',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-brand-profiles.component.html',
  styleUrls: ['../admin-shared.css'],
  encapsulation: ViewEncapsulation.None,
})
export class AdminBrandProfilesComponent implements OnInit {
  items:  any[] = [];
  ...
  form: any = {firstName:'', lastName:'', email:'', companyName:'', bio:'', isVerified:false, isActive:true, _id: ''};
  ...
  load(): void {
    this.loading = true;
    this.svc.list('brand-profiles', {search: this.search||undefined, page: this.page}).subscribe({
      next: (r:any) => { this.items=r.data||[]; this.total=r.total||0; this.pages=r.pages||1; this.loading=false; },
      error: () => { this.loading=false; },
    });
  }

  openCreate(): void { this.isEdit=false; this.form={firstName:'', lastName:'', email:'', companyName:'', bio:'', isVerified:false, isActive:true, _id:''}; this.showModal=true; }
  openEdit(item:any): void { this.isEdit=true; this.form={...item}; this.showModal=true; }

  save(): void {
    this.saving=true;
    const o$ = this.isEdit ? this.svc.update('brand-profiles', this.form._id, this.form) : this.svc.create('brand-profiles', this.form);
    o$.subscribe({
  ...
  doDelete(): void {
    this.svc.remove('brand-profiles', this.delId).subscribe({
