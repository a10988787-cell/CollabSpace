// src/app/Admin/admin-analytics/admin-analytics.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-analytics.component.css', '../admin-shared.css'],
...
export class AdminAnalyticsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('analytics').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('analytics', this.form._id, body)
      : this.svc.create('analytics', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('analytics', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('analytics_chart') as HTMLCanvasElement;
    if (!el) return;
