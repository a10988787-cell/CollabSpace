...
@Component({
  selector: 'app-admin-creator-profiles',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-creator-profiles.component.html',
  styleUrls: ['../admin-shared.css'],
  encapsulation: ViewEncapsulation.None,
})
export class AdminCreatorProfilesComponent implements OnInit {
  items:  any[] = [];
  ...
  form: any = {username:'', bio:'', niche:'Other', country:'', isVerified:false, _id: ''};
  ...
  load(): void {
    this.loading = true;
    this.svc.list('creator-profiles', {search: this.search||undefined, page: this.page}).subscribe({
      next: (r:any) => { this.items=r.data||[]; this.total=r.total||0; this.pages=r.pages||1; this.loading=false; },
      error: () => { this.loading=false; },
    });
  }

  openCreate(): void { this.isEdit=false; this.form={username:'', bio:'', niche:'Other', country:'', isVerified:false, _id:''}; this.showModal=true; }
  openEdit(item:any): void { this.isEdit=true; this.form={...item}; this.showModal=true; }

  save(): void {
    this.saving=true;
    const o$ = this.isEdit ? this.svc.update('creator-profiles', this.form._id, this.form) : this.svc.create('creator-profiles', this.form);
    o$.subscribe({
  ...
  doDelete(): void {
    this.svc.remove('creator-profiles', this.delId).subscribe({
