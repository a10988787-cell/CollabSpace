...
@Component({
  ...
  styleUrls: ['./admin-payments.component.css', '../admin-shared.css'],
...
export class AdminPaymentsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('payments').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('payments', this.form._id, body)
      : this.svc.create('payments', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('payments', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('payments_chart') as HTMLCanvasElement;
    if (!el) return;
