...
@Component({
  selector: 'app-admin-performance-analytics',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-performance-analytics.component.html',
  styleUrls: ['../admin-shared.css'],
  encapsulation: ViewEncapsulation.None,
})
export class AdminPerformanceAnalyticsComponent implements OnInit {
  items:  any[] = [];
  ...
  form: any = {platform:'Instagram', period:'30d', followers:0, engagementRate:0, avgLikes:0, avgComments:0, reach:0, impressions:0, _id: ''};
  ...
  load(): void {
    this.loading = true;
    this.svc.list('performance-analytics', {search: this.search||undefined, page: this.page}).subscribe({
      next: (r:any) => { this.items=r.data||[]; this.total=r.total||0; this.pages=r.pages||1; this.loading=false; },
      error: () => { this.loading=false; },
    });
  }

  openCreate(): void { this.isEdit=false; this.form={platform:'Instagram', period:'30d', followers:0, engagementRate:0, avgLikes:0, avgComments:0, reach:0, impressions:0, _id:''}; this.showModal=true; }
  openEdit(item:any): void { this.isEdit=true; this.form={...item}; this.showModal=true; }

  save(): void {
    this.saving=true;
    const o$ = this.isEdit ? this.svc.update('performance-analytics', this.form._id, this.form) : this.svc.create('performance-analytics', this.form);
    o$.subscribe({
  ...
  doDelete(): void {
    this.svc.remove('performance-analytics', this.delId).subscribe({
