// src/app/Admin/admin-subscriptions/admin-subscriptions.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-subscriptions.component.css', '../admin-shared.css'],
...
export class AdminSubscriptionsComponent implements OnInit {
  ...
  openEdit(item: any) {
    this.isEdit = true;
    this.form = { ...item, features: Array.isArray(item.features) ? [...item.features] : (item.features ? item.features.split(",").map((x:any)=>x.trim()) : []) };
    this.showModal = true;
  }
  ...
  save() {
    ...
    delete body._id;
    body.features = Array.isArray(body.features) ? body.features.join(',') : body.features;
    const obs = this.isEdit
      ? this.svc.update('plans', this.form._id, body)
      : this.svc.create('plans', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('plans', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('plans_chart') as HTMLCanvasElement;
    if (!el) return;
