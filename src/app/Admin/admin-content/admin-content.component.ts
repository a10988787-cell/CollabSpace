...
@Component({
  ...
  styleUrls: ['./admin-content.component.css', '../admin-shared.css'],
...
export class AdminContentComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('content').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('content', this.form._id, body)
      : this.svc.create('content', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('content', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('content_chart') as HTMLCanvasElement;
    if (!el) return;
