// src/app/Admin/admin-audit-logs/admin-audit-logs.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-audit-logs.component.css', '../admin-shared.css'],
...
export class AdminAuditLogsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('audit-logs').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('audit-logs', this.form._id, body)
      : this.svc.create('audit-logs', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('audit-logs', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('auditlogs_chart') as HTMLCanvasElement;
    if (!el) return;
