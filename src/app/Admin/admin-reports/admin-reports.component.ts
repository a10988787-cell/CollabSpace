...
@Component({
  ...
  styleUrls: ['./admin-reports.component.css', '../admin-shared.css'],
...
export class AdminReportsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('reports').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('reports', this.form._id, body)
      : this.svc.create('reports', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('reports', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('reports_chart') as HTMLCanvasElement;
    if (!el) return;
