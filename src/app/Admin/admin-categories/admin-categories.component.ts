...
@Component({
  ...
  styleUrls: ['./admin-categories.component.css', '../admin-shared.css'],
...
export class AdminCategoriesComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('categories').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('categories', this.form._id, body)
      : this.svc.create('categories', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('categories', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('categories_chart') as HTMLCanvasElement;
    if (!el) return;
