// src/app/Admin/admin-feature-flags/admin-feature-flags.component.ts
...
@Component({
  ...
  styleUrls: ['./admin-feature-flags.component.css', '../admin-shared.css'],
...
export class AdminFeatureFlagsComponent implements OnInit {
  ...
  load() {
    this.loading = true;
    this.svc.list('feature-flags').subscribe({
      next: r => {
  ...
  save() {
    ...
    const obs = this.isEdit
      ? this.svc.update('feature-flags', this.form._id, body)
      : this.svc.create('feature-flags', body);
    obs.subscribe({
  ...
  del() {
    this.svc.remove('feature-flags', this.delId).subscribe({
  ...
  initChart() {
    ...
    if (!Chart) return;
    const el = document.getElementById('featureflags_chart') as HTMLCanvasElement;
    if (!el) return;
